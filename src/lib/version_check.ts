...
import { CHECK_VERSION_EVERY_MILLIS, CURRENT_ACTOR_CLI_VERSION } from './consts.js';
...
export const SKIP_UPDATE_CHECK =
	process.env.ACTOR_CLI_SKIP_UPDATE_CHECK &&
	!['0', 'false'].includes(process.env.ACTOR_CLI_SKIP_UPDATE_CHECK.toLowerCase());

/**
 * Detect through which package manager the Actor CLI was installed.
...
/**
 * Fetches the latest NPM version of Actor CLI and caches it locally.
 */
const getAndCacheLatestNpmVersion = async (): Promise<string | undefined> => {
	try {
		info({ message: 'Making sure that Actor CLI is up to date...' });
		...
		console.error(err);
		warning({ message: 'Cannot fetch the latest Actor CLI version from NPM, using the cached version instead.' });
...
export const checkLatestVersion = async (enforceUpdate = false) => {
	...
	if (latestNpmVersion && gt(latestNpmVersion as string, CURRENT_ACTOR_CLI_VERSION)) {
		const installationType = detectInstallationType();
		const updateCommand = `' ${UPDATE_COMMAND[installationType]} '`;
		simpleLog({ message: '' });
		warning({
			message:
				'You are using an old version of Actor CLI. We strongly recommend you always use the latest available version.',
		});
		...
		info({ message: 'Actor CLI is up to date 👍 \n' });
	}
};
