...
import { ensureFolderExistsSync } from './files.js';
		...
		const answer = await inquirer.prompt([
			{
				...
				message: `The new version of Actor CLI uses the "${LOCAL_CONFIG_PATH}" instead of the "apify.json" file. Your "apify.json" file will be automatically updated to the new format under "${LOCAL_CONFIG_PATH}". The original file will be renamed by adding the ".deprecated" suffix. Do you want to continue?`,
			},
		]);
		if (!answer.isConfirm) {
			throw new Error(
...
export const setLocalEnv = async (actDir: string) => {
	...
	if (gitignoreAdditions.length > 0) {
		if (gitignoreContents.length > 0) {
			gitignoreAdditions.unshift('# Added by Actor CLI');
